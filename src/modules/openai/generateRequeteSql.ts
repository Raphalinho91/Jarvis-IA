...
async function generateRequeteSql(messages: Message[]): Promise<string> {
  try {
    messages.push({
      role: "system",
      content: `You have access to this database, which includes a "House" table. Note that the name of the "House" table is case-sensitive and must be used exactly as it is written with quotation marks ("House"). The "House" table has the following attributes:
        - city (varchar)
